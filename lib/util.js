...
exports.reverseByteOrder = function(buff){
    for (var i = 0; i < 8; i++) buff.writeUInt32LE(buff.readUInt32LE(i * 4), i * 4);
...
exports.varIntBuffer = function(n){
    if (n < 0xfd)
        ...
        buff[0] = 0xff;
        exports.packUInt16BE(n).copy(buff, 1);
        return buff;
    }
};
...
exports.serializeString = function(s){
    ...
    if (s.length < 253)
        ...
        return Buffer.concat([
            new Buffer([253]),
            exports.packUInt16BE(s.length),
            new Buffer(s)
        ]);
    else if (s.length < 0x100000000)
        return Buffer.concat([
            new Buffer([254]),
            exports.packUInt32BE(s.length),
            new Buffer(s)
        ]);
    else
        return Buffer.concat([
...
exports.bignumFromBitsHex = function(bitsString){
    var bitsBuffv2 = new Buffer(bitsString, 'hex');
    var bitsBuff = exports.packUInt32BE(bitsBuffv2)
