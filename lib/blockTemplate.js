...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    this.hashstaterootReversed = util.reverseByteOrder(new Buffer(rpcData.hashstateroot, 'hex')).toString('hex');
    this.hashutxorootReversed = util.reverseByteOrder(new Buffer(rpcData.hashutxoroot, 'hex')).toString('hex');
    ...
    this.serializeHeader = function(merkleRoot, nTime, nonce){
        ...
        var position = 0;
        header.writeUInt32LE(nonce, position, 4, 'hex');
        header.writeUInt32LE(rpcData.bits, position += 4, 4, 'hex');
        header.writeUInt32LE(nTime, position += 4, 4, 'hex');
        header.writeUInt32LE(merkleRoot, position += 4, 32, 'hex');
        header.writeUInt32LE(rpcData.hashstateroot, position += 32, 32, 'hex');
        header.writeUInt32LE(rpcData.hashutxoroot, position += 32, 32, 'hex');
        header.writeUInt32LE(rpcData.previousblockhash, position += 32, 32, 'hex');
    ...
    this.getJobParams = function(){
        if (!this.jobParams){
            this.jobParams = [
                this.jobId,
                util.packInt32LE(this.prevHashReversed).toString('hex'),
                util.packInt32LE(this.generationTransaction[0].toString('hex')).toString('hex'),
                util.packInt32LE(this.generationTransaction[1].toString('hex')).toString('hex'),
                util.packInt32LE(this.merkleBranch).toString('hex'),
                util.packInt32LE(this.rpcData.version).toString('hex'),
                util.packInt32LE(this.rpcData.bits).toString('hex'),
                util.packInt32LE(this.rpcData.curttime).toString('hex'),
                util.packInt32LE(this.hashstaterootReversed).toString('hex'),
                util.packInt32LE(this.hashutxorootReversed).toString('hex'),
                true
            ];
        }
