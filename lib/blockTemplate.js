...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    this.serializeHeader = function(merkleRoot, nTime, nonce){
        ...
        var position = 0;
        header.writeInt32LE(nonce, position, 4, 'hex');
        header.writeInt32LE(rpcData.bits, position += 4, 4, 'hex');
        header.writeInt32LE(nTime, position += 4, 4, 'hex');
        header.writeInt32LE(merkleRoot, position += 4, 32, 'hex');
        header.writeInt32LE(rpcData.hashstateroot, position += 32, 32, 'hex');
        header.writeInt32LE(rpcData.hashutxoroot, position += 32, 32, 'hex');
        header.writeInt32LE(rpcData.previousblockhash, position += 32, 32, 'hex');
